...
export default function Auth({mode,onAuth}:{mode:"login"|"register";onAuth:()=>void}) {
 ...
 async function submit(e:FormEvent){e.preventDefault();setError("");setBusy(true);try{if(register) await api.register(name,email,password); const r=await api.login(email,password);session.set(r.token);nav("/onboarding");onAuth();}catch(err){setError(err instanceof Error?err.message:"Unable to continue");}finally{setBusy(false);}}
 ...
 <label><span>Password</span><div className="input-wrap"><LockKeyhole size={17}/><input type="password" value={password} onChange={e=>setPassword(e.target.value)} required minLength={8} placeholder="At least 8 characters"/></div></label>
