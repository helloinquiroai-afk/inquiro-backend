...
class ConversationServiceTest {
    ...
    void setup() {
        ...
        var availability =
                new AvailabilityService(
                        List.of(
                                new BusinessKnowledgeAvailabilitySource()
                        )
                );
        ...
        var orchestrator =
                new InquiryOrchestrator(
                        ai,
                        slots,
                        new RequestAnalysisValidator(),
                        provider,
                        new BusinessQuestionService(ai),
                        availability,
                        boundary
                );
        ...
        when(
                channels.findByTypeAndExternalId(
                        any(),
                        anyString()
                )
        ).thenAnswer(invocation ->
                new BusinessChannel(
                        "ch-" + invocation.getArgument(1),
                        "biz-" + invocation.getArgument(1),
                        invocation.getArgument(0),
        ...
        when(accounts.findByBusinessId(anyString()))
                .thenAnswer(invocation ->
                        new BusinessAccount(
                                invocation.getArgument(0),
        ...
        /*
         * Existing conversation tests represent a fully configured
         * business, so onboarding is complete by default.
         */
        when(
                onboardingService.getOnboardingSummary(anyString())
        ).thenReturn(
    ...
    private void followUp(
        ...
        when(
                ...
                new FollowUpAnalysis(entities)
        );
    }

    @Test
    void accumulatesMultipleFieldsAndCreatesOnePendingRequest() {

        start();

        followUp(
                "Next Friday for two adults",
                Map.of(
                        "checkInDate",
                        "next Friday",
                        "guestCount",
        ...
        assertEquals(
                2,
                result.inquiry().fields().get("guestCount")
        );
        ...
        verify(requests).create(
                ...
                eq("ROOM_BOOKING"),
                argThat(fields ->
                        fields.get("guestCount").equals(2)
                                && fields.get("durationNights").equals(3)
                ),
    ...
    void explicitCorrectionsWin() {
        ...
        followUp(
                "Actually make it three adults",
                Map.of("guestCount", 3)
        );

        var result =
                say("Actually make it three adults");
        ...
        assertEquals(
                "next Friday",
                result.inquiry().fields().get("checkInDate")
        );
    ...
    @Test
    void businessQuestionPreservesUnfinishedRequest() {
        ...
        assertEquals(
                "Yes, parking is free.",
                say("Do you have free parking?").reply()
        );
    ...
    @Test
    void sameCustomerNeverSharesConversationAcrossPagesOrChannels() {
    ...
    @Test
    void rejectsDisabledChannelBeforeAi() {
    ...
    @Test
    void unknownRequestAsksForClarificationEvenWithHighConfidence() {
    ...
    @Test
    void mergeDoesNotEraseUsefulValuesOrMutateInputs() {
    ...
    @Test
    void identityRoundTripsWithoutDelimiterCollisions() {
        ...
        assertEquals(
                identity,
                ConversationIdentity.fromSessionId(
                        identity.sessionId()
                )
        );
    ...
    @Test
    void initialMixedRequestAnswersKnowledgeAndRetainsBookingEntities() {

        String message =
                "I need a room in Paris next Friday for 2 people "
    ...
    // =========================================================
    // PHASE 21A.3 - RECEPTIONIST READINESS
    // =========================================================

    @Test
    void blocksReceptionistWhenBusinessOnboardingIsIncomplete() {
    ...
    @Test
    void receptionistWorksWhenBusinessOnboardingIsComplete() {
